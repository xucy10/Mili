 ...
 * Mili code paths consume the most tick time.
 *
 * <p>用法 / Usage:
 * <pre>
 ...
 * </pre>
 *
 * <p>通过 /mili-perf profiler 或 dumpStats() 查看聚合统计 / Aggregated stats via
 * /mili-perf profiler or dumpStats().
 */
@ConfigClassInfo(category = EnumConfigCategory.OPTIMIZATIONS, name = "mili_tick_profiler")
public class MiliTickProfiler implements IConfigModule {
    @TransformedConfig(name = "enabled", directory = {"perf", "tick_profiler"})
    @ConfigInfo(name = "enabled", comments = """
            Enable Mili's lightweight tick profiler. When enabled, the
            Mili codebase measures its own hot paths (bot operations,
    ...
    public void onLoaded(@Nullable CommentedFileConfig configInstance, @Nullable Set<Exception> exs) {
        if (enabled) {
            LOGGER.info("mili tick profiler: ENABLED. Cost ~50ns/sample. Dump with /mili-perf (planned).");
        }
    }
