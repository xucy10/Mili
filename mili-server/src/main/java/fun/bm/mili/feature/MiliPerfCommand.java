...
/**
 * mili - /mili-perf 性能命令 / Performance diagnostics command.
 *
 * <p>子命令 / Subcommands:
 * <ul>
 *   <li>{@code /mili-perf status} - 显示所有性能模块状态 / Show all perf module settings</li>
 *   <li>{@code /mili-perf affinity} - CPU 亲和性状态 / CPU affinity status</li>
 *   <li>{@code /mili-perf regions} - 区域负载摘要 (前 20 区域) / Region load summary (top 20)</li>
 *   <li>{@code /mili-perf profiler} - Tick 探查器统计 / Tick profiler statistics</li>
 *   <li>{@code /mili-perf deadlock-stats} - 死锁/慢 tick 警告 / Deadlock/slow tick warnings</li>
 * </ul>
 *
 * <p>别名 / Aliases: {@code /lperf}, {@code /miliperf}.
 */
public class MiliPerfCommand extends RootNode {
    ...
    public MiliPerfCommand() {
        super("mili-perf", "mili.commands.perf");
    }
    ...
    public boolean requires(@NotNull CommandSourceStack source) {
        return source.getSender().hasPermission("mili.commands.perf");
    }
    ...
    private void sendStatus(CommandSourceStack stack) {
        ...
        stack.getSender().sendMessage(key("Tick 探查器", MiliTickProfiler.enabled
                ...
                ? NamedTextColor.GREEN : NamedTextColor.RED));
        stack.getSender().sendMessage(Component.text("  用法: /mili-perf {status|affinity|regions|profiler|deadlock-stats}")
    ...
    private void sendProfiler(CommandSourceStack stack) {
        ...
        if (!MiliTickProfiler.enabled) {
            stack.getSender().sendMessage(Component.text("  探查器未启用 (在 mili-perf.tick-profiler = true 启用)")
                    .color(NamedTextColor.YELLOW));
            return;
        }
