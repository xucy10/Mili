...
public class CreativeModeTabs {
            ...
            CreativeModeTab.builder(CreativeModeTab.Row.TOP, 3)
                ...
                .displayItems(
                    (parameters, output) -> {
                        ...
                        output.accept(Items.MAGENTA_BANNER);
                        output.accept(Items.PINK_BANNER);
                        output.accept(Raid.getOminousBannerInstance(parameters.holders().lookupOrThrow(Registries.BANNER_PATTERN)));
    ...
    public static void validate() {
        ...
        for (ResourceKey<CreativeModeTab> resourceKey : BuiltInRegistries.CREATIVE_MODE_TAB.registryKeySet()) {
            ...
            String string1 = map.put(Pair.of(creativeModeTab.row(), creativeModeTab.column()), string);
            if (string1 != null) {
                throw new IllegalArgumentException("Duplicate position: " + string + " vs. " + string1);
            }
        }
    }
    ...
    private static void generateEnchantmentBookTypesOnlyMaxLevel(
        ...
        enchantments.listElements()
            .map(reference -> EnchantmentHelper.createBook(new EnchantmentInstance(reference, reference.value().getMaxLevel())))
            .forEach(itemStack -> output.accept(itemStack, tabVisibility));
    }
    ...
    private static void generateEnchantmentBookTypesAllLevels(
        ...
        enchantments.listElements()
            .flatMap(
                reference -> IntStream.rangeClosed(reference.value().getMinLevel(), reference.value().getMaxLevel())
    ...
    private static void generateInstrumentTypes(
        ...
        instruments.get(instrument)
            .ifPresent(
                named -> named.stream()
                    .map(holder -> InstrumentItem.create(item, (Holder<Instrument>)holder))
                    .forEach(itemStack -> output.accept(itemStack, tabVisibility))
            );
    }
    ...
    private static void generatePresetPaintings(
        ...
        paintingVariants.listElements().filter(predicate).sorted(PAINTING_COMPARATOR).forEach(reference -> {
    ...
    private static void buildAllTabContents(CreativeModeTab.ItemDisplayParameters parameters) {
        streamAllTabs()
            ...
            .forEach(creativeModeTab -> creativeModeTab.buildContents(parameters));
        streamAllTabs()
            .filter(creativeModeTab -> creativeModeTab.getType() != CreativeModeTab.Type.CATEGORY)
            .forEach(creativeModeTab -> creativeModeTab.buildContents(parameters));
    }
