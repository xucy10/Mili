...
public class StandingAndWallBlockItem extends BlockItem {
    ...
    protected @Nullable BlockState getPlacementState(BlockPlaceContext context) {
        ...
        // CraftBukkit start
        if (blockState != null) {
            boolean defaultReturn = (fun.bm.mili.config.modules.function.CreativeFlyNoClipConfig.enabled && context.getPlayer() != null) ? context.getPlayer().canSpectatingPlace(level, blockState, clickedPos, CollisionContext.empty()) : level.isUnobstructed(blockState, clickedPos, CollisionContext.empty()); // Leaves - creative no clip
