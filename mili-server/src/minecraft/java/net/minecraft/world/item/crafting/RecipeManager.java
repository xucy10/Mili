...
public class RecipeManager extends SimplePreparableReloadListener<RecipeMap> implements RecipeAccess {
    ...
    protected void apply(RecipeMap object, ResourceManager resourceManager, ProfilerFiller profiler) {
        this.injectCompatRecipes(object);
        this.recipes = object;
        LOGGER.info("Loaded {} recipes", object.values().size());
    }

    /**
     * Re-inject compat recipes into the current recipe map after a config reload.
     * Called by CarpetCompatSync when carpet compat config is reloaded.
     * This removes previously injected compat recipes and re-adds them based on current config.
     */
    @SuppressWarnings("unchecked")
    public void reinjectCompatRecipes() {
        // Remove previously injected compat recipes
        this.recipes.removeRecipe((ResourceKey<Recipe<RecipeInput>>) (ResourceKey) BETTER_CRAFTABLE_BONE_BLOCK_RECIPE);
        this.recipes.removeRecipe((ResourceKey<Recipe<RecipeInput>>) (ResourceKey) BETTER_CRAFTABLE_DISPENSER_SHAPED_RECIPE);
        this.recipes.removeRecipe((ResourceKey<Recipe<RecipeInput>>) (ResourceKey) BETTER_CRAFTABLE_DISPENSER_SHAPELESS_RECIPE);

        // Re-inject based on current config
        this.injectCompatRecipes(this.recipes);

        // Rebuild derived collections (propertySets, allDisplays, recipeToDisplay, etc.)
        // and sync recipe book to clients. No-op if featureflagset is null (during early init,
        // derived collections will be rebuilt later by finalizeRecipeLoading(FeatureFlagSet)).
        this.finalizeRecipeLoading();

        LOGGER.info("Re-injected compat recipes (boneBlock={}, dispenser={})",
            fun.bm.mili.carpet.config.modules.GeneralCompatConfig.betterCraftableBoneBlock,
            fun.bm.mili.carpet.config.modules.GeneralCompatConfig.betterCraftableDispenser);
    }

    private void injectCompatRecipes(RecipeMap recipeMap) {
        if (fun.bm.mili.carpet.config.modules.GeneralCompatConfig.betterCraftableBoneBlock) {
            addCompatRecipe(recipeMap, createBetterCraftableBoneBlockRecipe());
        }

        if (fun.bm.mili.carpet.config.modules.GeneralCompatConfig.betterCraftableDispenser) {
    ...
    private static void addCompatRecipe(RecipeMap recipeMap, RecipeHolder<?> recipeHolder) {
    ...
    private static RecipeHolder<?> createBetterCraftableBoneBlockRecipe() {
        return new RecipeHolder<>(
            ...
            new ShapedRecipe(
                "carpet_ams_better_bone_block",
                CraftingBookCategory.BUILDING,
                ShapedRecipePattern.of(Map.of('#', Ingredient.of(net.minecraft.world.item.Items.BONE)), "###", "###", "###"),
                new net.minecraft.world.item.ItemStack(net.minecraft.world.item.Items.BONE_BLOCK, 3)
            )
        );
    }
    ...
    private static RecipeHolder<?> createBetterCraftableDispenserShapedRecipe() {
        return new RecipeHolder<>(
            ...
            new ShapedRecipe(
                ...
                new net.minecraft.world.item.ItemStack(net.minecraft.world.item.Items.DISPENSER)
            )
        );
    }
    ...
    private static ResourceKey<Recipe<?>> compatRecipeKey(String path) {
        return ResourceKey.create(Registries.RECIPE, Identifier.fromNamespaceAndPath("mili", path));
    }
