...
public class ObjectiveCriteria {
    ...
    public static final Codec<ObjectiveCriteria> CODEC = Codec.STRING
        .comapFlatMap(
            string -> byName(string).map(DataResult::success).orElse(DataResult.error(() -> "No scoreboard criteria with name: " + string)),
        ...
        if (objectiveCriteria != null) {
            ...
            return index < 0
                ...
                : BuiltInRegistries.STAT_TYPE
                    .getOptional(Identifier.bySeparator(name.substring(0, index), '.'))
                    .flatMap(statType -> getStat((StatType<?>)statType, Identifier.bySeparator(name.substring(index + 1), '.')));
        }
    }
