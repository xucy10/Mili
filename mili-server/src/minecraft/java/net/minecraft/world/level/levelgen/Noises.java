...
public class Noises {
    ...
    private static ResourceKey<NormalNoise.NoiseParameters> createKey(String key) {
        return ResourceKey.create(Registries.NOISE, Identifier.withDefaultNamespace(key));
    }

    public static NormalNoise instantiate(
        HolderGetter<NormalNoise.NoiseParameters> noises, PositionalRandomFactory random, ResourceKey<NormalNoise.NoiseParameters> resourceKey
    ) {
        Holder<NormalNoise.NoiseParameters> orThrow = noises.getOrThrow(resourceKey);
        return NormalNoise.create(random.fromHashOf(orThrow.unwrapKey().orElseThrow().identifier()), orThrow.value());
    }
}
