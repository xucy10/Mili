...
public final class BelowZeroRetrogen {
    ...
    private static final Codec<BitSet> BITSET_CODEC = Codec.LONG_STREAM
        .xmap(bitset -> BitSet.valueOf(bitset.toArray()), bitset -> LongStream.of(bitset.toLongArray()));
    private static final Codec<ChunkStatus> NON_EMPTY_CHUNK_STATUS = BuiltInRegistries.CHUNK_STATUS
        ...
        .comapFlatMap(
            chunkStatus -> chunkStatus == ChunkStatus.EMPTY
                ...
                : DataResult.success((ChunkStatus)chunkStatus),
            Function.identity()
        );
    ...
    public void applyBedrockMask(ProtoChunk chunk) {
        ...
        for (int i = 0; i < 16; i++) {
            for (int i1 = 0; i1 < 16; i1++) {
                if (this.hasBedrockHole(i, i1)) {
                    BlockPos.betweenClosed(i, minY, i1, i, maxY, i1).forEach(blockPos -> chunk.setBlockState(blockPos, Blocks.AIR.defaultBlockState()));
                }
            }
        }
    }
    ...
    public static BiomeResolver getBiomeResolver(BiomeResolver resolver, ChunkAccess chunk) {
        if (!chunk.isUpgrading()) {
            return resolver;
        } else {
            Predicate<ResourceKey<Biome>> predicate = RETAINED_RETROGEN_BIOMES::contains;
            return (x, y, z, sampler) -> {
                Holder<Biome> noiseBiome = resolver.getNoiseBiome(x, y, z, sampler);
                return noiseBiome.is(predicate) ? noiseBiome : chunk.getNoiseBiome(x, 0, z);
            };
        }
    }
}
