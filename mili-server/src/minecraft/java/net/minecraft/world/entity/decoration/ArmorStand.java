    ...
    public InteractionResult interactAt(Player player, Vec3 vec, InteractionHand hand) {
        ...
        if (this.isMarker() || itemInHand.is(Items.NAME_TAG)) {
            ...
            if (itemInHand.isEmpty()) {
                ...
                // Mili start - stick can change ArmorStand arm status
                if (fun.bm.mili.config.modules.function.OldFeatureConfig.stickChangeArmorStandArmStatus && itemInHand.is(Items.STICK) && player.isShiftKeyDown()) {
                    setShowArms(!showArms());
                    return InteractionResult.FAIL;
                }
                // Mili end - stick can change ArmorStand arm status
