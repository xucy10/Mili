...
public class MemoryModuleType<U> {
    ...
    public MemoryModuleType(Optional<Codec<U>> optionalCodec) {
        this.codec = optionalCodec.map(ExpirableValue::codec);
    }
    ...
    private static <U> MemoryModuleType<U> register(String identifier, Codec<U> codec) {
        return Registry.register(BuiltInRegistries.MEMORY_MODULE_TYPE, Identifier.withDefaultNamespace(identifier), new MemoryModuleType<>(Optional.of(codec)));
    }

    private static <U> MemoryModuleType<U> register(String identifier) {
        return Registry.register(BuiltInRegistries.MEMORY_MODULE_TYPE, Identifier.withDefaultNamespace(identifier), new MemoryModuleType<>(Optional.empty()));
    }
}
