...
public class Inventory implements Container, Nameable {
    ...
    public int canHold(ItemStack itemStack) {
        ...
        for (int slot = 0; slot < this.items.size(); ++slot) {
            ...
            if (this.hasRemainingSpaceForItem(itemInSlot, itemStack)) {
                remains -= (org.leavesmc.leaves.util.ItemOverstackUtils.getItemStackMaxCount(itemInSlot) < this.getMaxStackSize() ? org.leavesmc.leaves.util.ItemOverstackUtils.getItemStackMaxCount(itemInSlot) : this.getMaxStackSize()) - itemInSlot.getCount(); // Mili - item over-stack util
            }
            if (remains <= 0) {
                return itemStack.getCount();
            }
        ...
        if (this.hasRemainingSpaceForItem(itemInOffhand, itemStack)) {
            remains -= (org.leavesmc.leaves.util.ItemOverstackUtils.getItemStackMaxCount(itemInOffhand) < this.getMaxStackSize() ? org.leavesmc.leaves.util.ItemOverstackUtils.getItemStackMaxCount(itemInOffhand) : this.getMaxStackSize()) - itemInOffhand.getCount(); // Mili - item over-stack util
        }
        if (remains <= 0) {
            return itemStack.getCount();
        }
    ...
    private int addResource(int slot, ItemStack stack) {
        ...
        int i = this.getMaxStackLeaves(item) - item.getCount(); // Mili - item over-stack util
    ...
    public void placeItemBackInInventory(ItemStack stack, boolean sendPacket) {
        while (!stack.isEmpty()) {
            ...
            int i = org.leavesmc.leaves.util.ItemOverstackUtils.getItemStackMaxCount(stack) - this.getItem(slotWithRemainingSpace).getCount(); // Mili - item over-stack util
