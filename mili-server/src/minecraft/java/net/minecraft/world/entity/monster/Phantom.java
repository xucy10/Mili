...
public class Phantom extends Mob implements Enemy {
    ...
    protected void dropFromLootTable(ServerLevel level, DamageSource source, boolean causedByPlayer) {
        ...
        if (fun.bm.mili.config.modules.function.OldFeatureConfig.renewableElytra > 0.0D) {
            if (source.getEntity() instanceof Shulker && this.random.nextDouble() < fun.bm.mili.config.modules.function.OldFeatureConfig.renewableElytra) {
