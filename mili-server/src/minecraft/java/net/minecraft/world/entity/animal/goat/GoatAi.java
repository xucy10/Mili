...
public class GoatAi {
    ...
    private static final TargetingConditions RAM_TARGET_CONDITIONS = TargetingConditions.forCombat()
        .selector(
            (entity, level) -> !entity.getType().equals(EntityType.GOAT)
                && (level.getGameRules().get(GameRules.MOB_GRIEFING) || !entity.getType().equals(EntityType.ARMOR_STAND))
    ...
    public static void updateActivity(Goat brain) {
        brain.getBrain().setActiveActivityToFirstValid(ImmutableList.of(Activity.RAM, Activity.LONG_JUMP, Activity.IDLE));
    }
}
