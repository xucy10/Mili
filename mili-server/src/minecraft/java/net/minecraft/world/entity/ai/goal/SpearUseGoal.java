        ...
        if (this.state != null) {
            ...
            if (this.state.notEngagedYet()) {
                if (d > this.approachDistanceSq) {
                    this.mob.getNavigation().moveTo(target, f * this.speedModifierWhenRepositioning);
                    return;
                }
            ...
            if (this.state.tickAndCheckEngagement()) {
                ...
                this.state.awayPos = LandRandomPos.getPosAway(
                    this.mob, Math.max(0.0, 9 + i - squareRoot), Math.max(1.0, 11 + i - squareRoot), 7, target.position()
                );
            ...
            if (!this.state.tickAndCheckFleeing()) {
                if (this.state.awayPos != null) {
                    ...
                    this.mob.getNavigation().moveTo(target, f * this.speedModifierWhenCharging);
                    if (d < this.targetInRangeRadiusSq || this.mob.getNavigation().isDone()) {
                        double squareRoot = Math.sqrt(d);
                        this.state.awayPos = LandRandomPos.getPosAway(this.mob, 6 + i - squareRoot, 7 + i - squareRoot, 7, target.position());
                    }
                }
            }
        }
