...
public class StrollAroundPoi {
    ...
    public static OneShot<PathfinderMob> create(MemoryModuleType<GlobalPos> poiPosMemory, float speedModifier, int maxDistFromPoi) {
            ...
            instance -> instance.group(instance.registered(MemoryModuleType.WALK_TARGET), instance.present(poiPosMemory))
                .apply(instance, (walkTarget, poiPos) -> (level, mob, gameTime) -> {
                    ...
                    if (level.dimension() != globalPos.dimension() || !globalPos.pos().closerToCenterThan(mob.position(), maxDistFromPoi)) {
                        ...
                        Optional<Vec3> optional = Optional.ofNullable(LandRandomPos.getPos(mob, 8, 6));
                        walkTarget.setOrErase(optional.map(targetPos -> new WalkTarget(targetPos, speedModifier, 1)));
                        mutableLong.setValue(gameTime + 180L);
                        return true;
                    }
                })
        );
