...
public class Spider extends Monster {
    ...
    // Mili start - spider jockeys drop gapples
    @Override
    protected void dropFromLootTable(net.minecraft.server.level.ServerLevel level, DamageSource damageSource, boolean causedByPlayer) {
        ...
        if (fun.bm.mili.config.modules.function.VillagerTradeConfig.spiderJockeysDropGapples > 0.0D) {
            if (this.hasPassenger(entity -> entity.getType() == EntityType.SKELETON) && this.random.nextDouble() < fun.bm.mili.config.modules.function.VillagerTradeConfig.spiderJockeysDropGapples) {
                this.spawnAtLocation(level, new net.minecraft.world.item.ItemStack(net.minecraft.world.item.Items.ENCHANTED_GOLDEN_APPLE));
            }
        }
    }
    // Mili end - spider jockeys drop gapples
