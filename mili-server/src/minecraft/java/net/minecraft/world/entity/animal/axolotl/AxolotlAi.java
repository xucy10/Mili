...
public class AxolotlAi {
    ...
    private static boolean canSetWalkTargetFromLookTarget(LivingEntity entity) {
        ...
        Optional<PositionTracker> memory = entity.getBrain().getMemory(MemoryModuleType.LOOK_TARGET);
        if (memory.isPresent()) {
            BlockPos blockPos = memory.get().currentBlockPosition();
    ...
    private static float getSpeedModifier(LivingEntity entity) {
        return entity.isInWater() ? 0.5F : 0.15F;
    }
