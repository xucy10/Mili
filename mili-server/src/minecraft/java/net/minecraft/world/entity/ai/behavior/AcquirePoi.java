                ...
                .apply(
                    ...
                    memoryAccessor -> (level, mob, time) -> {
                        if (onlyIfAdult && mob.isBaby()) {
                            ...
                            if (!fun.bm.mili.config.modules.function.TechnicalSurvivalModeConfig.enabled && level.paperConfig().entities.behavior.stuckEntityPoiRetryDelay.enabled() && mob.getNavigation().isStuck()) mutableLong.add(level.paperConfig().entities.behavior.stuckEntityPoiRetryDelay.intValue()); // Paper - Next stuck check delay config // Mili - mc technical survival mode
