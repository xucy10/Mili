...
public class ItemStackTheFlatteningFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("ItemInstanceTheFlatteningFix", type, typed -> {
            ...
            if (optional.isEmpty()) {
                ...
                int _int = dynamic.get("Damage").asInt(0);
                String string = updateItem(optional.get().getSecond(), _int);
                if (string != null) {
    ...
    public static @Nullable String updateItem(@Nullable String item, int dataValue) {
        if (IDS.contains(item)) {
            String string = MAP.get(item + "." + dataValue);
            return string == null ? MAP.get(item + ".0") : string;
        } else {
            return null;
        }
    }
