...
public class BlockStateData {
    ...
    public static void register(int index, Dynamic<?> defaultData, Dynamic<?>... others) {
        MAP[index] = defaultData;
        int i = index >> 4;
        if (BLOCK_DEFAULTS[i] == null) {
            BLOCK_DEFAULTS[i] = defaultData;
        }

        for (Dynamic<?> dynamic : others) {
            String string = dynamic.get("Name").asString("");
            ID_BY_OLD_NAME.putIfAbsent(string, index);
            ID_BY_OLD.put(dynamic, index);
        }
    }
    ...
    public static Dynamic<?> upgradeBlockStateTag(Dynamic<?> tag) {
        int _int = ID_BY_OLD.getInt(tag);
        if (_int >= 0 && _int < MAP.length) {
            Dynamic<?> dynamic = MAP[_int];
            return dynamic == null ? tag : dynamic;
        } else {
            return tag;
        }
    }

    public static String upgradeBlock(String name) {
        int _int = ID_BY_OLD_NAME.getInt(name);
        if (_int >= 0 && _int < MAP.length) {
            Dynamic<?> dynamic = MAP[_int];
