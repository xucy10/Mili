...
public class StatsCounterFix extends DataFix {
    ...
    private static StatsCounterFix.@Nullable StatType unpackLegacyKey(String key) {
        if (SKIP.contains(key)) {
            ...
            String string = CUSTOM_MAP.get(key);
            if (string != null) {
                ...
                if (i < 0) {
                    ...
                    if ("stat.mineBlock".equals(sub)) {
                        String string1 = upgradeBlock(key.substring(i + 1).replace('.', ':'));
                        return new StatsCounterFix.StatType("minecraft:mined", string1);
                    } else {
                        String string1 = ITEM_KEYS.get(sub);
                        if (string1 != null) {
                            String string2 = key.substring(i + 1).replace('.', ':');
                            String string3 = upgradeItem(string2);
                            String string4 = string3 == null ? string2 : string3;
                            return new StatsCounterFix.StatType(string1, string4);
                        } else {
                            String string2 = ENTITY_KEYS.get(sub);
                            if (string2 != null) {
                                String string3 = key.substring(i + 1).replace('.', ':');
                                String string4 = ENTITIES.getOrDefault(string3, string3);
                                return new StatsCounterFix.StatType(string2, string4);
                            } else {
                                return null;
                            }
                        }
            ...
            if (optional.isPresent()) {
                for (Entry<? extends Dynamic<?>, ? extends Dynamic<?>> entry : optional.get().entrySet()) {
                    if (entry.getValue().asNumber().result().isPresent()) {
                        ...
                        if (statType != null) {
                            Dynamic<?> dynamic1 = dynamic.createString(statType.type());
                            Dynamic<?> dynamic2 = map.computeIfAbsent(dynamic1, dynamic3 -> dynamic.emptyMap());
                            map.put(dynamic1, dynamic2.set(statType.typeKey(), (Dynamic<?>)entry.getValue()));
                        }
                    }
                }
            }
    ...
    private TypeRewriteRule makeObjectiveFixer() {
        ...
        return this.fixTypeEverywhereTyped("ObjectiveStatFix", type, type1, typed -> {
            Dynamic<?> dynamic = typed.get(DSL.remainderFinder());
            Dynamic<?> dynamic1 = dynamic.update("CriteriaName", dynamic2 -> DataFixUtils.orElse(dynamic2.asString().result().map(string -> {
            ...
            }).map(dynamic2::createString), dynamic2));
            return Util.readTypedOrThrow(type1, dynamic1);
        });
    }

    private static @Nullable String upgradeItem(String id) {
