...
public class ItemPotionFix extends DataFix {
    ...
    private static final @Nullable String[] POTIONS = DataFixUtils.make(new String[128], strings -> {
            ...
            typed -> {
                ...
                if (optional.isPresent() && Objects.equals(optional.get().getSecond(), "minecraft:potion")) {
                    ...
                    if (optionalTyped.isPresent()) {
                        Typed<?> typed1 = typed;
                        Dynamic<?> dynamic1 = optionalTyped.get().get(DSL.remainderFinder());
                        Optional<String> optional1 = dynamic1.get("Potion").asString().result();
                        if (optional1.isEmpty()) {
                            String string = POTIONS[_short & 127];
                            Typed<?> typed2 = optionalTyped.get()
                                .set(DSL.remainderFinder(), dynamic1.set("Potion", dynamic1.createString(string == null ? "minecraft:water" : string)));
                            typed1 = typed.set(opticFinder1, typed2);
                            if ((_short & 16384) == 16384) {
