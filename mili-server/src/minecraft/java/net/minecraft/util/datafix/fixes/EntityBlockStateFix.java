...
public class EntityBlockStateFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        Function<Typed<?>, Typed<?>> function1 = typed -> this.updateBlockToBlockState(typed, "inTile", "inData", "inBlockState");
        ...
        Function<Typed<?>, Typed<?>> function2 = typed -> typed.update(type.finder(), DSL.remainderType(), Pair::getSecond);
        return this.fixTypeEverywhereTyped(
            ...
            typed -> {
                ...
                typed = this.updateEntity(
                    typed, "minecraft:enderman", typed1 -> this.updateBlockToBlockState(typed1, "carried", "carriedData", "carriedBlockState")
                );
    ...
    private Typed<?> updateFallingBlock(Typed<?> typed) {
        ...
        return typed.update(type.finder(), type1, either -> {
            int i = either.map(pair -> pair.getSecond().map(integer -> (Integer)integer, EntityBlockStateFix::getBlockId), unit -> {
            ...
            int i1 = dynamic.get("Data").asInt(0) & 15;
            return Either.left(Pair.of(References.BLOCK_STATE.typeName(), BlockStateData.getTag(i << 4 | i1)));
    ...
    private Typed<?> updateBlockToBlockState(Typed<?> typed, String tileKey, String dataKey, String outputKey) {
        ...
        return typed.update(type.finder(), type1, pair -> {
            int i = pair.getSecond().map(integer -> (Integer)integer, EntityBlockStateFix::getBlockId);
            int i1 = dynamic.get(dataKey).asInt(0) & 15;
            return Pair.of(References.BLOCK_STATE.typeName(), BlockStateData.getTag(i << 4 | i1));
    ...
    private Typed<?> updateEntity(Typed<?> typed, String choiceName, Function<Typed<?>, Typed<?>> updater) {
        ...
        return typed.updateTyped(DSL.namedChoice(choiceName, choiceType), choiceType1, updater);
    }
}
