...
public class ChunkStructuresTemplateRenameFix extends DataFix {
    ...
    private Dynamic<?> fixChildren(Dynamic<?> dynamic) {
        return dynamic.update("Children", dynamic1 -> dynamic.createList(dynamic1.asStream().map(dynamic2 -> this.fixTag(dynamic, (Dynamic<?>)dynamic2))));
    }

    private Dynamic<?> fixTag(Dynamic<?> parent, Dynamic<?> child) {
        String string = parent.get("id").asString("");
        if (RENAMES.containsKey(string)) {
            Pair<String, ImmutableMap<String, String>> pair = RENAMES.get(string);
            if (pair.getFirst().equals(child.get("id").asString(""))) {
                String string1 = child.get("Template").asString("");
                child = child.set("Template", child.createString(pair.getSecond().getOrDefault(string1, string1)));
            }
        }
