...
public class AttributeModifierIdFix extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return TypeRewriteRule.seq(
            this.fixTypeEverywhereTyped(
                ...
                type,
                typed -> typed.updateTyped(opticFinder, typed1 -> typed1.update(DSL.remainderFinder(), AttributeModifierIdFix::fixItemStackComponents))
            ),
    ...
    private static Stream<Dynamic<?>> fixModifiers(Stream<Dynamic<?>> modifiers) {
        ...
        modifiers.forEach(dynamic -> {
            ...
            String string1 = uuid != null ? ID_MAP.get(uuid) : null;
            String string2 = NAME_MAP.get(string);
            if (string1 != null) {
                dynamic = dynamic.set("id", dynamic.createString(string1));
                map.put(string1, dynamic.remove("uuid").remove("name"));
            } else if (string2 != null) {
                Dynamic<?> dynamic1 = map.get(string2);
                if (dynamic1 == null) {
                    dynamic = dynamic.set("id", dynamic.createString(string2));
                    map.put(string2, dynamic.remove("uuid").remove("name"));
                } else {
                    double _double = dynamic1.get("amount").asDouble(0.0);
                    double _double1 = dynamic.get("amount").asDouble(0.0);
                    map.put(string2, dynamic1.set("amount", dynamic.createDouble(_double + _double1)));
                }
            } else {
                String string3 = "minecraft:" + (uuid != null ? uuid.toString().toLowerCase(Locale.ROOT) : "unknown");
    ...
    private static Dynamic<?> convertModifierForEntity(Dynamic<?> modifier) {
        return modifier.renameField("UUID", "uuid")
            ...
            .renameAndFixField("Operation", "operation", dynamic -> {
                return dynamic.createString(switch (dynamic.asInt(0)) {
                    case 0 -> "add_value";
    ...
    private static Dynamic<?> fixItemStackComponents(Dynamic<?> tag) {
        return tag.update(
            "minecraft:attribute_modifiers",
            dynamic -> dynamic.update(
                "modifiers",
                dynamic1 -> DataFixUtils.orElse(
                    dynamic1.asStreamOpt().result().map(AttributeModifierIdFix::fixModifiersTypeWrapper).map(dynamic1::createList), dynamic1
                )
            )
        );
    }

    private static Dynamic<?> fixAttribute(Dynamic<?> tag) {
        return tag.renameField("Name", "id")
    ...
    private static Typed<?> fixEntity(Typed<?> entity) {
        return entity.update(
            ...
            dynamic -> dynamic.renameAndFixField(
                ...
                "attributes",
                dynamic1 -> DataFixUtils.orElse(
                    dynamic1.asStreamOpt().result().map(stream -> stream.map(AttributeModifierIdFix::fixAttribute)).map(dynamic1::createList), dynamic1
                )
            )
        );
    }
