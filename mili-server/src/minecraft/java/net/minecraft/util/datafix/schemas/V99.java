...
public class V99 extends Schema {
    ...
    public Map<String, Supplier<TypeTemplate>> registerEntities(Schema schema) {
        ...
        schema.register(map, "Item", string -> DSL.optionalFields("Item", References.ITEM_STACK.in(schema)));
        ...
        schema.register(map, "Arrow", string -> DSL.optionalFields("inTile", References.BLOCK_NAME.in(schema)));
        schema.register(map, "TippedArrow", string -> DSL.optionalFields("inTile", References.BLOCK_NAME.in(schema)));
        schema.register(map, "SpectralArrow", string -> DSL.optionalFields("inTile", References.BLOCK_NAME.in(schema)));
        ...
        schema.register(
            map, "ThrownPotion", string -> DSL.optionalFields("inTile", References.BLOCK_NAME.in(schema), "Potion", References.ITEM_STACK.in(schema))
        );
        registerThrowableProjectile(schema, map, "ThrownExpBottle");
        schema.register(map, "ItemFrame", string -> DSL.optionalFields("Item", References.ITEM_STACK.in(schema)));
        registerThrowableProjectile(schema, map, "WitherSkull");
        schema.registerSimple(map, "PrimedTnt");
        schema.register(
            map, "FallingSand", string -> DSL.optionalFields("Block", References.BLOCK_NAME.in(schema), "TileEntityData", References.BLOCK_ENTITY.in(schema))
        );
        schema.register(map, "FireworksRocketEntity", string -> DSL.optionalFields("FireworksItem", References.ITEM_STACK.in(schema)));
        ...
        schema.register(map, "Enderman", string -> DSL.optionalFields("carried", References.BLOCK_NAME.in(schema)));
    ...
    public Map<String, Supplier<TypeTemplate>> registerBlockEntities(Schema schema) {
        ...
        schema.register(map, "RecordPlayer", string -> DSL.optionalFields("RecordItem", References.ITEM_STACK.in(schema)));
        registerInventory(schema, map, "Trap");
        registerInventory(schema, map, "Dropper");
        schema.register(map, "Sign", () -> sign(schema));
        schema.register(map, "MobSpawner", string -> References.UNTAGGED_SPAWNER.in(schema));
        ...
        schema.register(map, "FlowerPot", string -> DSL.optionalFields("Item", DSL.or(DSL.constType(DSL.intType()), References.ITEM_NAME.in(schema))));
    ...
    protected static <T> T addNames(Dynamic<T> tag, Map<String, String> blockEntityRenames, Map<String, String> entityRenames) {
        return tag.update("tag", dynamic -> dynamic.update("BlockEntityTag", dynamic1 -> {
            String string = tag.get("id").asString().result().map(NamespacedSchema::ensureNamespaced).orElse("minecraft:air");
            if (!"minecraft:air".equals(string)) {
                String string1 = blockEntityRenames.get(string);
                if (string1 != null) {
            ...
            if (dynamic1.get("id").result().isPresent()) {
                ...
                String string1 = entityRenames.get(string);
