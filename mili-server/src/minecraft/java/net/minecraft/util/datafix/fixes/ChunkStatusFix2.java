...
public class ChunkStatusFix2 extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped(
            "ChunkStatusFix2", type, this.getOutputSchema().getType(References.CHUNK), typed -> typed.updateTyped(opticFinder, typed1 -> {
                Dynamic<?> dynamic = typed1.get(DSL.remainderFinder());
                String string = dynamic.get("Status").asString("empty");
                String string1 = RENAMES_AND_DOWNGRADES.getOrDefault(string, "empty");
                return Objects.equals(string, string1) ? typed1 : typed1.set(DSL.remainderFinder(), dynamic.set("Status", dynamic.createString(string1)));
            })
        );
    }
}
