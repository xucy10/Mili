...
public class DebugSubscriptions<T> {
    ...
    private static DebugSubscription<?> registerSimple(String name) {
        return Registry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, Identifier.withDefaultNamespace(name), new DebugSubscription(null));
    }

    private static <T> DebugSubscription<T> registerWithValue(String name, StreamCodec<? super RegistryFriendlyByteBuf, T> valueStreamCodec) {
        return Registry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, Identifier.withDefaultNamespace(name), new DebugSubscription<>(valueStreamCodec));
    }

    private static <T> DebugSubscription<T> registerTemporaryValue(
        String name, StreamCodec<? super RegistryFriendlyByteBuf, T> valueStreamCodec, int expireAfterTicks
    ) {
        return Registry.register(
            BuiltInRegistries.DEBUG_SUBSCRIPTION, Identifier.withDefaultNamespace(name), new DebugSubscription<>(valueStreamCodec, expireAfterTicks)
        );
    }
}
