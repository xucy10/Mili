...
public class VanillaRegistries {
    ...
    public static void validateThatAllBiomeFeaturesHaveBiomeFilter(HolderGetter<PlacedFeature> features, HolderLookup<Biome> biomes) {
        biomes.listElements().forEach(reference -> {
            Identifier identifier = reference.key().identifier();
            List<HolderSet<PlacedFeature>> list = reference.value().getGenerationSettings().features();
            list.stream().flatMap(HolderSet::stream).forEach(holder -> holder.unwrap().ifLeft(resourceKey -> {
                Holder.Reference<PlacedFeature> orThrow = features.getOrThrow((ResourceKey<PlacedFeature>)resourceKey);
                if (!validatePlacedFeature(orThrow.value())) {
                    Util.logAndPauseIfInIde("Placed feature " + resourceKey.identifier() + " in biome " + identifier + " is missing BiomeFilter.biome()");
                }
    ...
    private static boolean validatePlacedFeature(PlacedFeature feature) {
        return feature.placement().contains(BiomeFilter.biome());
    }
