...
public class ShearsDispenseItemBehavior extends OptionalDispenseItemBehavior {
    ...
    protected ItemStack execute(BlockSource blockSource, ItemStack item) {
        ...
        if (!serverLevel.isClientSide()) {
            ...
            if (this.isSuccess()) {
                item.hurtAndBreak(1, serverLevel, null, item1 -> { if (fun.bm.mili.config.modules.function.OldFeatureConfig.shearsInDispenserCanZeroAmount) item.grow(1); }); // Mili - Make shears in dispenser can unlimited use
            }
        }
