...
public class PlayerTrigger extends SimpleCriterionTrigger<PlayerTrigger.TriggerInstance> {
    ...
    public record TriggerInstance(@Override Optional<ContextAwarePredicate> player) implements SimpleCriterionTrigger.SimpleInstance {
        ...
        public static Criterion<PlayerTrigger.TriggerInstance> walkOnBlockWithEquipment(
            HolderGetter<Block> blockRegistry, HolderGetter<Item> itemRegistry, Block block, Item equipment
        ) {
            return located(
                EntityPredicate.Builder.entity()
                    .equipment(EntityEquipmentPredicate.Builder.equipment().feet(ItemPredicate.Builder.item().of(itemRegistry, equipment)))
                    .steppingOn(LocationPredicate.Builder.location().setBlock(BlockPredicate.Builder.block().of(blockRegistry, block)))
            );
        }
    }
}
